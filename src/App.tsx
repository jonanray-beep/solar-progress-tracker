...
function forecastFinishDate(args: {
  ...
  while (remaining > 0 && daysUsed < 365) {
    if (isWorkday(cursor)) {
  ...
  const projectedFinish = daysUsed >= 365 ? null : cursor;
...
async function fetchWeatherForLocation(location: string): Promise<WeatherDay[]> {
  ...
  return workdayDates.map((date) => {
    ...
    return { id: uid(), date, type: weatherCodeToType(index >= 0 ? codes[index] : 0), note: "", source: "api" };
  });
}

function runSelfTests() {
  if (workdaysBetween("2026-04-20", "2026-04-24") !== 5) throw new Error("Self-test failed: workdaysBetween");
  if (addWorkdays("2026-04-25", 1) !== "2026-04-27") throw new Error("Self-test failed: addWorkdays");
...
function Stat({ label, value, sub, tone = "slate" }: { label: string; value: string; sub?: string; tone?: "slate" | "green" | "amber" | "red" | "blue" }) {
  const colors = {
    ...
    <div className="min-h-screen bg-slate-100 px-4 py-10 text-slate-900">
      <div className="mx-auto max-w-md rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mb-4 flex gap-2">
          <Button secondary={mode !== "signin"} onClick={() => setMode("signin")}>Sign in</Button>
...
function DashboardTab({ activeProject, projectMetrics, recommendation }: { activeProject: Project; projectMetrics: ReturnType<typeof getProjectMetrics>; recommendation: string }) {
  return (
    <div className="space-y-4">
      ...
      <Card title="Progress against schedule" subtitle="Reality vs target finish">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 p-4">
            ...
            <div className="mt-2"><ProgressBar value={projectMetrics.percentComplete} /></div>
        ...
        return (
          <Card key={task.id} title={task.name} subtitle={`Task chart • ${task.category} • ${task.unit}`}>
            <div className="mb-4 grid gap-3 md:grid-cols-5">
              ...
              <Stat label="People needed/day" value={fmt(metrics.requiredHeadcount, 1)} sub={metrics.usingReality ? "actual productivity" : "target productivity"} tone="blue" />
...
function TasksTab({ activeProject, canEdit, isAdmin, addTask, removeTask, updateProject }: { activeProject: Project; canEdit: boolean; isAdmin: boolean; addTask: () => void; removeTask: (taskId: string) => void; updateProject: (projectId: string, updater: (project: Project) => Project) => void }) {
  return (
    <Card title="Task management" subtitle="Per-project quantity, dates, and target productivity" action={<Button disabled={!canEdit} onClick={addTask}>Add task</Button>}>
      <div className="space-y-4">
        {activeProject.tasks.map((task) => {
          const metrics = getTaskMetrics(activeProject, task);
          return (
            <div key={task.id} className="rounded-3xl border border-slate-200 p-4">
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <Field label="Task name"><Input disabled={!canEdit} value={task.name} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, name: event.target.value } : item) }))} /></Field>
                <Field label="Category"><Input disabled={!canEdit} value={task.category} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, category: event.target.value } : item) }))} /></Field>
                <Field label="Planned quantity"><Input disabled={!canEdit} type="number" value={task.plannedQty} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, plannedQty: safeNonNegativeNumber(event.target.value, 0) } : item) }))} /></Field>
                <Field label="Baseline complete"><Input disabled={!canEdit} type="number" value={task.baselineCompleteQty} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, baselineCompleteQty: safeNonNegativeNumber(event.target.value, 0) } : item) }))} /></Field>
                <Field label="Unit"><Input disabled={!canEdit} value={task.unit} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, unit: event.target.value } : item) }))} /></Field>
                <Field label="Start date"><Input disabled={!canEdit} type="date" value={task.startDate} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, startDate: event.target.value } : item) }))} /></Field>
                <Field label="Target finish"><Input disabled={!canEdit} type="date" value={task.targetFinish} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, targetFinish: event.target.value } : item) }))} /></Field>
                <Field label="Target productivity / person / day"><Input disabled={!canEdit} type="number" step="0.1" value={task.targetProductivityPerPerson} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, targetProductivityPerPerson: Math.max(0.01, safeNonNegativeNumber(event.target.value, 0.01)) } : item) }))} /></Field>
              </div>
              <div className="mt-3"><Field label="Notes"><TextArea disabled={!canEdit} value={task.notes} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.map((item) => item.id === task.id ? { ...item, notes: event.target.value } : item) }))} /></Field></div>
...
function EntriesTab({ activeProject, canEdit, addDailyEntry, removeDailyEntry, toggleApplyEntry, updateProject }: { activeProject: Project; canEdit: boolean; addDailyEntry: () => void; removeDailyEntry: (entryId: string) => void; toggleApplyEntry: (entry: DailyEntry) => void; updateProject: (projectId: string, updater: (project: Project) => Project) => void }) {
  return (
    <Card title="Daily production entries" subtitle="Unapplied entries do not affect task progress or projections." action={<Button disabled={!canEdit} onClick={addDailyEntry}>Add entry</Button>}>
      <div className="space-y-4">
        {[...activeProject.dailyEntries].sort((a, b) => b.date.localeCompare(a.date)).map((entry) => (
          <div key={entry.id} className="grid gap-3 rounded-3xl border border-slate-200 p-4 lg:grid-cols-[1fr,1fr,1fr,1fr,1.2fr,auto,auto]">
            <Field label="Date"><Input disabled={!canEdit} type="date" value={entry.date} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => item.id === entry.id ? { ...item, date: event.target.value } : item) }))} /></Field>
            <Field label="Task"><Select disabled={!canEdit} value={entry.taskId || "all"} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => item.id === entry.id ? { ...item, taskId: event.target.value === "all" ? null : event.target.value, applied: false } : item) }))}><option value="all">Project-level only</option>{activeProject.tasks.map((task) => <option key={task.id} value={task.id}>{task.name}</option>)}</Select></Field>
            <Field label="Quantity"><Input disabled={!canEdit} type="number" value={entry.qty} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => item.id === entry.id ? { ...item, qty: safeNonNegativeNumber(event.target.value, 0) } : item) }))} /></Field>
            <Field label="Headcount"><Input disabled={!canEdit} type="number" value={entry.headcount} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => item.id === entry.id ? { ...item, headcount: safeNonNegativeNumber(event.target.value, 0) } : item) }))} /></Field>
            <Field label="Notes"><Input disabled={!canEdit} value={entry.note} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => item.id === entry.id ? { ...item, note: event.target.value } : item) }))} /></Field>
            <div className="flex items-end"><Button disabled={!canEdit} onClick={() => toggleApplyEntry(entry)}>{entry.applied ? "Applied" : "Apply"}</Button></div>
            <div className="flex items-end"><Button secondary disabled={!canEdit} onClick={() => removeDailyEntry(entry.id)}>Delete</Button></div>
          </div>
        ))}
      </div>
    </Card>
...
function WeatherTab({ activeProject, canEdit, weatherLoading, weatherError, refreshWeather, updateProject }: { activeProject: Project; canEdit: boolean; weatherLoading: boolean; weatherError: string; refreshWeather: () => Promise<void>; updateProject: (projectId: string, updater: (project: Project) => Project) => void }) {
  return (
    <Card title="10-day weather cycle" subtitle="Weather affects forward-looking projections." action={<Button disabled={!canEdit || weatherLoading} onClick={() => void refreshWeather()}>{weatherLoading ? "Refreshing..." : "Refresh weather"}</Button>}>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {activeProject.weather.map((day) => (
          <div key={day.id} className="grid gap-3 rounded-3xl border border-slate-200 p-4 md:grid-cols-3">
            <Field label="Date"><Input disabled={!canEdit} type="date" value={day.date} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, weather: project.weather.map((item) => item.id === day.id ? { ...item, date: event.target.value } : item) }))} /></Field>
            <Field label="Condition"><Select disabled={!canEdit} value={day.type} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, weather: project.weather.map((item) => item.id === day.id ? { ...item, type: event.target.value as WeatherType, source: "manual" } : item) }))}>{WEATHER_TYPES.map((type) => <option key={type.value} value={type.value}>{type.label} ({type.factor}x)</option>)}</Select></Field>
            <Field label="Note"><Input disabled={!canEdit} value={day.note} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, weather: project.weather.map((item) => item.id === day.id ? { ...item, note: event.target.value } : item) }))} /></Field>
            <div className="text-sm text-slate-500 md:col-span-3">Impact factor: <strong>{weatherMeta(day.type).factor}x</strong> • {day.source === "api" ? "From forecast" : "Manual override"}</div>
          </div>
        ))}
      </div>
    </Card>
...
function SettingsTab({ activeProject, canEdit, updateProject }: { activeProject: Project; canEdit: boolean; updateProject: (projectId: string, updater: (project: Project) => Project) => void }) {
  return (
    <Card title="Program settings" subtitle="Tune forecasting and persistence behavior">
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Project start date"><Input disabled={!canEdit} type="date" value={activeProject.startDate} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, startDate: event.target.value }))} /></Field>
        <Field label="Target finish"><Input disabled={!canEdit} type="date" value={activeProject.targetFinish} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, targetFinish: event.target.value }))} /></Field>
        <Field label="Project status"><Select disabled={!canEdit} value={activeProject.status} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, status: event.target.value }))}><option value="Active">Active</option><option value="Delayed">Delayed</option><option value="Complete">Complete</option></Select></Field>
        <Field label="Trailing workdays used for recent reality"><Input disabled={!canEdit} type="number" min="1" value={activeProject.settings.trailingDays} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, settings: { ...project.settings, trailingDays: safePositiveInteger(event.target.value, 5) } }))} /></Field>
      </div>
      <label className="mt-4 flex items-center justify-between rounded-2xl border border-slate-200 p-3">
        <div><div className="font-medium">Use weather in projection</div><div className="text-sm text-slate-500">Apply the 10-day forecast to forward-looking production and finish dates.</div></div>
        <input disabled={!canEdit} type="checkbox" checked={activeProject.settings.useWeatherInProjection} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, settings: { ...project.settings, useWeatherInProjection: event.target.checked } }))} />
      </label>
    </Card>
  );
}
...
function AppShell() {
  ...
  async function handleAuthSubmit() {
    ...
    try {
      if (authMode === "signup") {
        const { error } = await withTimeout(client.auth.signUp({ email: authEmail, password: authPassword, options: AUTH_REDIRECT_URL ? { emailRedirectTo: AUTH_REDIRECT_URL } : undefined }), 12000, "Create account timed out");
        if (error) throw error;
        setAuthMessage("Account created. Check your email if confirmation is required, then sign in.");
        setAuthMode("signin");
      } else {
        const { error } = await withTimeout(client.auth.signInWithPassword({ email: authEmail, password: authPassword }), 12000, "Sign in timed out");
        if (error) throw error;
        setAuthMessage("");
      }
    } catch (error: any) {
  ...
  function addTask() {
    ...
    updateProject(activeProject.id, (project) => ({ ...project, tasks: [...project.tasks, { id: uid(), name: `New Task ${project.tasks.length + 1}`, category: "General", startDate: todayISO(), targetFinish: addWorkdays(todayISO(), 10), plannedQty: 100, baselineCompleteQty: 0, unit: "units", active: true, notes: "", targetProductivityPerPerson: 5 }] }));
  }

  function removeTask(taskId: string) {
    if (!activeProject || !isAdmin) return;
    updateProject(activeProject.id, (project) => ({ ...project, tasks: project.tasks.filter((task) => task.id !== taskId), dailyEntries: project.dailyEntries.filter((entry) => entry.taskId !== taskId) }));
  }

  function addDailyEntry() {
    if (!activeProject || !canEdit) return;
    updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: [...project.dailyEntries, { id: uid(), date: todayISO(), taskId: project.tasks[0]?.id || null, qty: 0, headcount: 0, note: "", applied: false }] }));
  }

  function removeDailyEntry(entryId: string) {
    if (!activeProject || !canEdit) return;
    updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.filter((entry) => entry.id !== entryId) }));
  }

  function toggleApplyEntry(entry: DailyEntry) {
    if (!activeProject || !canEdit) return;
    updateProject(activeProject.id, (project) => ({ ...project, dailyEntries: project.dailyEntries.map((item) => (item.id === entry.id ? { ...item, applied: !item.applied } : item)) }));
  }
        ...
        <div className="mb-6 grid gap-4 lg:grid-cols-[280px,1fr]">
          <aside className="rounded-3xl bg-slate-900 p-4 text-white shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <div><div className="text-xs uppercase tracking-[0.2em] text-slate-400">Power Progress</div><div className="text-xl font-semibold">Solar Tracker</div></div>
              <button onClick={addProject} disabled={!canEdit} className={`rounded-2xl bg-white/10 px-3 py-2 text-sm hover:bg-white/20 ${!canEdit ? "cursor-not-allowed opacity-40" : ""}`}>+ Project</button>
            </div>
            <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-3">
              ...
              {session?.user?.email ? <div className="mt-2 text-xs text-slate-300">Signed in as {session.user.email}</div> : null}
              <div className="mt-1 text-xs text-slate-300">Access: {currentRole}</div>
              {supabaseRef.current && session ? <button onClick={() => void handleSignOut()} className="mt-3 rounded-2xl border border-white/10 px-3 py-2 text-xs font-medium text-white hover:bg-white/10">Sign out</button> : null}
            </div>
            <div className="space-y-2">
              {data.projects.map((project) => (
                <button key={project.id} onClick={() => setActiveProjectId(project.id)} className={`w-full rounded-2xl border px-3 py-3 text-left transition ${project.id === activeProjectId ? "border-white/30 bg-white/10" : "border-white/10 bg-white/5 hover:bg-white/10"}`}>
                  <div className="flex items-start justify-between gap-3"><div><div className="font-medium">{project.name}</div><div className="mt-1 text-xs text-slate-300">{project.location || "No location set"}</div></div>{data.projects.length > 1 && isAdmin ? <span onClick={(event) => { event.stopPropagation(); removeProject(project.id); }} className="rounded-lg px-2 py-1 text-xs text-red-200 hover:bg-white/10">Delete</span> : null}</div>
                </button>
              ))}
            </div>
          </aside>
          <main className="space-y-4">
            <Card title={activeProject.name} subtitle="Superintendent schedule and production intelligence">
              <div className="grid gap-3 md:grid-cols-2"><Field label="Project name"><Input disabled={!canEdit} value={activeProject.name} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, name: event.target.value }))} /></Field><Field label="Location"><Input disabled={!canEdit} value={activeProject.location} onChange={(event) => updateProject(activeProject.id, (project) => ({ ...project, location: event.target.value }))} /></Field></div>
              <div className="mt-4 flex flex-wrap gap-2"><Badge tone="blue">{activeProject.status}</Badge><Badge>{activeProject.location || "Location not set"}</Badge><Badge>{activeProject.startDate} → {activeProject.targetFinish}</Badge><Badge tone="amber">5-day workweek</Badge></div>
            </Card>
            <div className="flex flex-wrap gap-2">{(["dashboard", "tasks", "entries", "weather", "users", "settings"] as TabKey[]).map((name) => <button key={name} onClick={() => setTab(name)} className={`rounded-2xl px-4 py-2 text-sm font-medium capitalize ${tab === name ? "bg-slate-900 text-white" : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50"}`}>{name}</button>)}</div>
            {tab === "dashboard" ? <DashboardTab activeProject={activeProject} projectMetrics={projectMetrics} recommendation={recommendation} /> : null}
            {tab === "tasks" ? <TasksTab activeProject={activeProject} canEdit={canEdit} isAdmin={isAdmin} addTask={addTask} removeTask={removeTask} updateProject={updateProject} /> : null}
            {tab === "entries" ? <EntriesTab activeProject={activeProject} canEdit={canEdit} addDailyEntry={addDailyEntry} removeDailyEntry={removeDailyEntry} toggleApplyEntry={toggleApplyEntry} updateProject={updateProject} /> : null}
            {tab === "weather" ? <WeatherTab activeProject={activeProject} canEdit={canEdit} weatherLoading={weatherLoading} weatherError={weatherError} refreshWeather={refreshWeather} updateProject={updateProject} /> : null}
            {tab === "users" ? <UsersTab users={data.users} isAdmin={isAdmin} hasConfiguredEmails={hasConfiguredEmails} usesSupabase={Boolean(supabaseRef.current)} addUser={addUser} removeUser={removeUser} setData={setData} /> : null}
            {tab === "settings" ? <SettingsTab activeProject={activeProject} canEdit={canEdit} updateProject={updateProject} /> : null}
          </main>
        </div>
      </div>
    </div>
